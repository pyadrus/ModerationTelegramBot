...
def ban(chatid, channelid):
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS channel (
            chat_id INTEGER PRIMARY KEY,  # Уникальный идентификатор чата
            channel_id INTEGER,  # Идентификатор канала
            block INTEGER DEFAULT 0  # Поле для указания, заблокирована ли подписка (0 - не заблокировано, 1 - заблокировано)
        )
    ''')
